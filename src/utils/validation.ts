...
export const milkCollectionSchema = Joi.object({
  ...
  fat_percentage: Joi.number().min(0).max(100).required(),
  snf_percentage: Joi.number().min(0).max(100).required(),
...
export const milkPriceSchema = Joi.object({
  ...
  fat_rate: Joi.number().required(),
  snf_rate: Joi.number().required(),
  milk_type: Joi.string().valid('cow', 'buffalo', 'mix_milk').default('mix_milk'),
